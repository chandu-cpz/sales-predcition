...
# Import the refactored YoloTracker class (now expected in the same directory)
try:
    from yolo_tracker import YoloTracker
except ImportError:
    print("Error: Could not import YoloTracker from yolo_tracker.py.")
    print("Ensure yolo_tracker.py is in the 'esp32' directory.")
    exit(1)

# Import suggestion functions (assuming suggestions.py is in the parent directory)
try:
    # Adjust path relative to the current script (esp32/)
    suggestions_script_path = os.path.join(os.path.dirname(__file__), '..')
    sys.path.insert(0, suggestions_script_path) # Temporarily add parent dir to path
    from suggestions import get_average_sales, generate_suggestion
    # Remove the path modification after import if desired, though often not necessary
    # sys.path.pop(0)
except ImportError:
    print("Error: Could not import suggestion functions from ../suggestions.py.")
    print("Ensure suggestions.py exists in the parent directory of 'esp32'.")
    exit(1)
except Exception as import_err:
     print(f"An unexpected error occurred during suggestion import: {import_err}")
     exit(1)
...
# Or use the hardcoded key (less secure):
GOOGLE_API_KEY = os.environ.get("GOOGLE_API_KEY", "AIzaSyAlV6jPPwaj669vdEigWnfyLBrel4xKWns") # Fallback to hardcoded
GOOGLE_MODEL_NAME = "gemini-1.5-flash-latest"

SERIAL_PORT = '/dev/ttyACM0'  # *** UPDATED to user's ESP32 serial port ***
BAUD_RATE = 115200
# Paths relative to this script's location (esp32/)
RFID_MAPPING_FILE = 'rfid_item_mapping.csv'
SALES_DATA_FILE = '../retail_data_full_with_people_time.csv' # Access parent directory

# YOLO Tracker Config (Paths relative to this script's location)
YOLO_MODEL_PATH = '../best.pt' # Assuming model is in the parent directory
VIDEO_SOURCE = '0' # Webcam index or video file path
SHOW_YOLO_VIDEO = True # Show the YOLO tracking visualization window
AVG_DWELL_PERIOD_MINUTES = 5 # Period for calculating average dwell time
YOLO_OUTPUT_CSV = 'yolov8_dwell_times.csv' # Save dwell times CSV within esp32/
...
def load_sales_data(filename=SALES_DATA_FILE):
    ...
    try:
        ...
        if not os.path.exists(filename):
             print(f"Error: Sales data file not found at the expected path: {os.path.abspath(filename)}")
             print("Ensure the script is run from the 'esp32' directory and the CSV is in the parent directory.")
             return False

        df = pd.read_csv(filename, encoding='ISO-8859-1') # Common encoding for retail data
        # More robust date parsing with error handling
        ...
        ).fillna(0).reset_index() # Fill NaNs resulting from resampling with 0

        # Need to re-fetch description after resampling
        # Handle duplicate StockCodes by keeping the first occurrence of each
        desc_map = df[['StockCode', 'Description']].drop_duplicates(subset=['StockCode']).set_index('StockCode')
        # Use map with a default value for missing StockCodes
        daily_sales['Description'] = daily_sales['StockCode'].map(desc_map['Description']).fillna('Unknown')
        # No need for separate fillna as we've already handled missing values
        ...
        # This specific exception might be less likely now due to the explicit check above
    ...
    try:
        with map_lock:
            if os.path.exists(filename):
                ...
                print(f"RFID mapping file '{filename}' not found. Starting with empty map.")
                rfid_map_df = pd.DataFrame(columns=['RFID_UID', 'StockCode', 'Description'])
        return True
    except Exception as e:
...
def create_lagged_features(timeseries_data, lags=PREDICTION_LAGS):
    ...
    # Check if columns exist before attempting to lag
    ...
    if df_lags.empty:
        print("Warning: DataFrame is empty after creating lags and dropping NaNs.")
        return None, None
            ...
            if serial_conn.in_waiting > 0:
                ...
                if line.startswith("RFID_UID:"):
                    ...
                    if not mapping_entry.empty:
                        ...
                        print(f"\n{Fore.CYAN}═══════════════ RFID TAG SCANNED ═══════════════{Style.RESET_ALL}")
                        print(f"{Fore.BLUE}RFID:{Style.RESET_ALL} {rfid_uid}")
                        print(f"{Fore.BLUE}Item:{Style.RESET_ALL} {stock_code} - {description}")
                        print(f"{Fore.CYAN}═══════════════════════════════════════════════{Style.RESET_ALL}\n")
                    else:
                        print(f"\n{Fore.MAGENTA}▓▒░ New RFID UID detected: {rfid_uid} ░▒▓{Style.RESET_ALL}")
                        print(f"{Fore.YELLOW}Finding best item to map...{Style.RESET_ALL}")
                        ...
                        if new_stock_code and new_description:
                            ...
                            print(f"\n{Fore.GREEN}▃▄▅▇█ NEW MAPPING CREATED █▇▅▄▃{Style.RESET_ALL}")
                            mapping_data = [[rfid_uid, stock_code, description]]
                            mapping_table = tabulate(
                                mapping_data,
                                headers=["RFID UID", "StockCode", "Description"],
                                tablefmt="pretty"
                            )
                            print(mapping_table)
                        else:
                            print(f"{Fore.RED}✘ Could not find an unmapped item for RFID {rfid_uid}. ✘{Style.RESET_ALL}")
                    ...
                    if stock_code:
                        ...
                        if predicted_need is not None:
                            # Enhanced colorful output with tabulate
                            prediction_data = [[stock_code, description, predicted_need]]
                            prediction_table = tabulate(
                                prediction_data, 
                                headers=[f"{Fore.CYAN}StockCode{Style.RESET_ALL}", f"{Fore.CYAN}Description{Style.RESET_ALL}", f"{Fore.CYAN}Predicted Need{Style.RESET_ALL}"],
                                tablefmt="pretty"
                            )
                            print(f"\n{Fore.GREEN}★ PREDICTION RESULT ★{Style.RESET_ALL}")
                            print(prediction_table)
                            print(f"{Fore.YELLOW}Recommendation: Order {predicted_need} units for tomorrow.{Style.RESET_ALL}")

                            # --- Generate and Print Suggestion ---
                            print(f"\n{Fore.MAGENTA}Generating Sales Suggestion...{Style.RESET_ALL}")
                            try:
                                avg_sales = get_average_sales(stock_code)
                                if avg_sales is not None: # Check if average calculation was successful
                                     print(f"Historical Average Sales: {avg_sales:.2f}")
                                     # Use predicted_need as the 'current' sales figure for the suggestion context
                                     suggestion = generate_suggestion(stock_code, predicted_need, avg_sales)
                                     print(f"\n{Fore.BLUE}--- AI Sales Suggestion ---{Style.RESET_ALL}")
                                     print(suggestion)
                                     print(f"{Fore.BLUE}---------------------------{Style.RESET_ALL}\n")
                                else:
                                     print(f"{Fore.YELLOW}Could not calculate average sales for {stock_code}. Skipping suggestion.{Style.RESET_ALL}")
                            except Exception as sugg_err:
                                 print(f"{Fore.RED}Error generating suggestion: {sugg_err}{Style.RESET_ALL}")
                            # --- End Suggestion ---

                        else:
                            print(f"{Fore.RED}Prediction failed for {stock_code}.{Style.RESET_ALL}")
...
def yolo_tracker_task():
    ...
    try:
        tracker = YoloTracker(
            model_path=YOLO_MODEL_PATH, # Path relative to esp32/
            ...
            output_csv=YOLO_OUTPUT_CSV # Save CSV within esp32/
        )
...
if __name__ == "__main__":
    ...
    print(f"Current working directory: {os.getcwd()}") # Should be esp32/

    # --- Configure Google AI ---
    if not GOOGLE_API_KEY:
        print(f"{Fore.RED}Error: GOOGLE_API_KEY not set as environment variable or hardcoded.{Style.RESET_ALL}")
        print("Please set the GOOGLE_API_KEY environment variable or add it to the script.")
        exit(1)
    try:
        print(f"Configuring Google Generative AI with model: {GOOGLE_MODEL_NAME}...")
        genai.configure(api_key=GOOGLE_API_KEY)
        # Test connection briefly (optional, but good practice)
        # model_check = genai.GenerativeModel(GOOGLE_MODEL_NAME)
        # model_check.generate_content("test") # Simple test prompt
        print(f"{Fore.GREEN}Google Generative AI configured successfully.{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}Error configuring Google Generative AI: {e}{Style.RESET_ALL}")
        print("Please check your API key and network connection.")
        exit(1)
    # --- End Google AI Config ---
